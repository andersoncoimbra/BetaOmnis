                            ...
                            @forelse($reembolsos as $reembolso)
                                <tr><td>{{$reembolso->parceiro}}</td><td>
                                        ...
                                        @if(strlen($reembolso->job) > 31)
                                            ...
                                            @endif
                                    </td><td>{{$reembolso->valor}}</td><td>{{date('d / m / Y', strtotime($reembolso->data))}}</td><td><input type="button" class="btn btn-info" value="Detalhes" onclick="detalhesreembolso({{$reembolso->id}})" style="margin-right: 2px; margin-left: 2px" ><input type="button" class="btn btn-primary" value="Check-in" onclick="checkinreembolso({{$reembolso->id}})" style="margin-right: 2px; margin-left: 2px" ><input type="button" class="btn btn-danger"  style="margin-right: 2px; margin-left: 2px" value="Quitação" onclick="formModal('compensar')"></td></tr>
                            @empty
