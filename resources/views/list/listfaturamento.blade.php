<table class="table table-striped">
    <tr><th>#ID</th><th>Parceiro</th><th>Job</th><th>Valor</th><th>Ações</th><td></td><td></td></tr>
    @forelse($faturamentos as $faturamento)
        ...
        @else
                success
        @endif
                "><td>{{$faturamento->id}}</td><td>{{$faturamento->parceiro}}</td><td>{{$faturamento->job}}</td><td>{{$faturamento->valor}}</td><td><input type="button" class="btn btn-info" value="Detalhes" onclick="formModal('detalhes', '{{$faturamento->id}}','faturamento')" style="margin-right: 2px; margin-left: 2px" ></td><td><input type="button" class="btn btn-primary" value="Nota Fiscal" onclick="formModal('nf','{{$faturamento->id}}', 'faturamento')" style="margin-right: 2px; margin-left: 2px" ></td><td>@if($faturamento->nf)<input type="button" class="btn btn-danger" value="Quitação" onclick="formModal('quitacao','{{$faturamento->id}}', 'faturamento')" style="margin-right: 2px; margin-left: 2px" > @endif</td></tr>
    @empty
